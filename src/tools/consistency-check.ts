...
export interface ConsistencyCheckParams {
  ...
  proposal: string;              // 当前方案摘要
  acceptanceCriteria?: string;   // 验收标准
  proposalPaths?: string[];      // 方案相关文件路径
  maxOutputTokens?: number;      // 输出 token 预算（默认 500）
}

// 冲突项接口
export interface ConflictItem {
  element_a: string;             // 冲突的一方
  element_b: string;             // 冲突的另一方
...
export interface ConsistencyCheckResult {
  ...
  // 未覆盖的需求（<=10）
  requirements_not_covered: Array<{
    requirement: string;
    source: 'goal' | 'constraint' | 'acceptance_criteria';
  }>;
  ...
  summary: {
    ...
    total_gaps: number;
    overall_consistency: 'consistent' | 'minor_issues' | 'major_issues' | 'inconsistent';
  };
 ...
 * 解析一致性检查响应
 */
function parseConsistencyCheckResponse(response: string): Partial<ConsistencyCheckResult> {
  try {
    ...
    const conflicts = (parsed.conflicts || []).slice(0, 10).map((c: any) => ({
    ...
    const requirements_not_covered = (parsed.requirements_not_covered || []).slice(0, 10).map((r: any) => ({
      requirement: r.requirement || '',
      source: validSources.includes(r.source) ? r.source : 'goal'
    }));
    
    const validation_gaps = (parsed.validation_gaps || []).slice(0, 10).map((g: any) => ({
      gap: g.gap || '',
...
function calculateOverallConsistency(
  ...
  uncovered: number,
  gaps: number
): 'consistent' | 'minor_issues' | 'major_issues' | 'inconsistent' {
  const total = conflicts + uncovered + gaps;
  
  if (total === 0) return 'consistent';
  if (conflicts === 0 && total <= 3) return 'minor_issues';
  if (conflicts <= 2 && total <= 6) return 'major_issues';
  return 'inconsistent';
}
