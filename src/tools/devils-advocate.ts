...
// 参数接口
export interface DevilsAdvocateParams {
  proposal: string;              // 当前方案摘要（建议 15-30 行）
  proposalPaths?: string[];      // 方案相关文件路径
  goal?: string;                 // 目标
  constraints?: string;          // 约束条件
  maxOutputTokens?: number;      // 输出 token 预算（默认 600）
}
 ...
 * 解析反对者响应
 */
function parseDevilsAdvocateResponse(response: string): Partial<DevilsAdvocateResult> {
  try {
    ...
    return {
      critical_risks: (parsed.critical_risks || []).slice(0, 10).map((r: any) => ({
        risk: r.risk || '',
        severity: validSeverities.includes(r.severity) ? r.severity : 'medium',
        impact: r.impact || ''
      })),
      hidden_assumptions: (parsed.hidden_assumptions || []).slice(0, 8).map((a: any) => ({
        assumption: a.assumption || '',
        why_problematic: a.why_problematic || ''
      })),
      missing_considerations: (parsed.missing_considerations || []).slice(0, 8).map((c: any) => ({
 ...
 */
export async function handleDevilsAdvocate(
  params: DevilsAdvocateParams,
  client: GeminiClient
  ...
  try {
    ...
    const totalIssues = 
      (parsed.critical_risks?.length || 0) +
