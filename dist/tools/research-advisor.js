 ...
 * 构建研究顾问提示词
 */
function buildResearchPrompt(params, materialsContent) {
...
function parseResearchResponse(response) {
    try {
        ...
        return {
            key_concepts: (parsed.key_concepts || []).slice(0, 8).map((c) => ({
                concept: c.concept || 'Unknown',
                explanation: c.explanation || '',
                source: c.source
            })),
            recommended_directions: (parsed.recommended_directions || []).slice(0, 5).map((d) => ({
                direction: d.direction || '',
                rationale: d.rationale || ''
            })),
            open_questions: (parsed.open_questions || []).slice(0, 5),
            best_practices: (parsed.best_practices || []).slice(0, 8).map((p) => ({
                practice: p.practice || '',
                context: p.context || '',
                source: p.source
            })),
 ...
 */
export async function handleResearchAdvisor(params, client) {
    try {
        // 参数验证
...
//# sourceMappingURL=research-advisor.js.map